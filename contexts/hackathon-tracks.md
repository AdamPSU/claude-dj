...
Strong project angle: use Deepgram as the DJ narration layer. Claude decides what to say, the MCP `narrate` tool turns it into a short spoken DJ line, and the mascot app can later show the same line as text. Preferred voice direction is a confident, human, personal music-guide persona with short contextual commentary, closer to Spotify DJ's public product style than a generic hype-man announcer; exact Aura voice/model is TBD after auditioning and API verification.
