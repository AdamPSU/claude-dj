...
- Use a soft colorkey blend for generated mascot video cutouts. Hard/binary keys leave white halos and jagged edges; verify the result against a high-contrast matte, not just `alpha_mode=1` metadata.
...
- For backend harness tests, prefer real API keys, real MCP/tool calls, and existing live integration paths when verifying runtime behavior. Use mocks only for pure logic/unit boundaries or when the user explicitly asks to avoid billing/external calls.
- Do not claim webcam/CV integration is verified unless `WebcamWorker.start()` or the real `claude_dj.main` startup path has run with the required local model assets installed. Constructing the runtime or running fake-camera E2E tests is not sufficient.
- When porting work from another branch, fetch the latest remote branch and identify the active production path before copying code. Do not assume demo scripts such as `test_face.py` represent the newest model pipeline.
- When a teammate references a newer ML model vaguely, search all refs and commit history for model/backbone names and ensemble code before porting. Do not stop at the latest visible entrypoint if the user hints at a more advanced implementation.
