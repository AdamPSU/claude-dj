...
- Resolve the CLAP checkpoint via `config.clap_checkpoint()` (call-time), not the import-time `config.CLAP_CHECKPOINT` constant, which is captured before `load_dotenv()` and ignores `.env`. The local checkpoint lives in `recommendation_engine/models/`; `clap_checkpoint()` resolves bare/relative names against the package root and `models/` so it works regardless of cwd. `load_model` also sets `SSL_CERT_FILE`/`REQUESTS_CA_BUNDLE` from certifi so laion-clap's macOS `urllib` download doesn't fail with `CERTIFICATE_VERIFY_FAILED`. A missing/mis-pathed checkpoint silently falls back to a network download; prefer the on-disk file.
