...
class ReactionScoringTests(unittest.TestCase):
    def test_negative_collapsed_emotions_score_negative(self) -> None:
        frame = ReactionFrame(
            ...
            movement=0.0,
            face=0.1,
            emotions={"positive": 0.0, "neutral": 0.2, "negative": 0.8},
        )
        baseline = Baseline(
            movement=0.3,
            face=0.5,
            emotions={"positive": 0.3, "neutral": 0.5, "negative": 0.2},
        )
    ...
    def test_capture_baseline_preserves_average_collapsed_emotions(self) -> None:
        frames = [
            ReactionFrame(movement=0.1, face=0.4, emotions={"positive": 0.2, "neutral": 0.6, "negative": 0.2}),
            ReactionFrame(movement=0.3, face=0.6, emotions={"positive": 0.4, "neutral": 0.4, "negative": 0.2}),
        ]
        ...
        self.assertAlmostEqual(baseline.emotions["positive"], 0.3)

    def test_emotion_confidence_increases_when_distribution_is_peaked(self) -> None:
        flat = emotion_confidence({"positive": 0.34, "neutral": 0.33, "negative": 0.33})
        peaked = emotion_confidence({"positive": 0.9, "neutral": 0.05, "negative": 0.05})
